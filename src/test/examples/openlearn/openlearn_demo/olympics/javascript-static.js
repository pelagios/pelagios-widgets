...
function addBookmark() {
	if (window.sidebar) {
	    document.getElementById('bookmark').innerHTML="Bookmark Page";
...
function unit_summary_show( counter, all_divs ) {
    if( all_divs ) {
        var summery_trimmed = new getObj( 'summery_trimmed_'+counter )
        var summery         = new getObj( 'summery_'+counter )
    }
    var collapse        = new getObj( 'collapse_'+counter )
    var expand          = new getObj( 'expand_'+counter )

    if( all_divs ) {
        summery_trimmed.style.display = 'none'
        summery.style.display = ''
    }
...
function unit_summary_hide( counter, all_divs ) {
    if( all_divs ) {
        var summery_trimmed = new getObj( 'summery_trimmed_'+counter )
        var summery         = new getObj( 'summery_'+counter )
    }
...
function elementToggleHideAll(el, classname){
	var obj = findParentNode(el, 'DIV', classname).parentNode;
 	var divs = obj.getElementsByTagName('div');
 	var exp = "(^|\\s)" + classname + "(\\s|$)";
 	var re = new RegExp(exp); 
 	
 	for(var i = 0; i<divs.length; i++){
 		if(divs[i].className.search(re) > -1 && divs[i].className.indexOf('hidden') == -1){
 			var inputs = divs[i].getElementsByTagName('input');
 			for(var x = 0; x<inputs.length; x++){
 				if(inputs[x].className.indexOf('togglecard')>-1 && inputs[x].id != el.id){
 					inputs[x].click();
 				}
 			}
 		}
